...
def main():
    ...
    # # greenimg
    # greenimg = np.array(rgbarray)
    # greenimg[:, :, ::2] = 0
    # surfdemo_show(greenimg, "greenimg")

    # # blueimg
    # blueimg = np.array(rgbarray)
    # blueimg[:, :, :2] = 0
    # surfdemo_show(blueimg, "blueimg")
    ...
    noise = 1
    f_n = 8
    factor = np.array((f_n,), int32)
    soften = np.array(rgbarray, int32)
    soften[noise:, :] += rgbarray[:-noise, :] * factor
    soften[:-noise, :] += rgbarray[noise:, :] * factor
    soften[:, noise:] += rgbarray[:, :-noise] * factor
    soften[:, :-noise] += rgbarray[:, noise:] * factor
    ...
    dest[:] = 20, 50, 100
    diff = (dest - src) * 0.50
    print(diff.astype(uint))
    xfade = src + diff.astype(uint)
    print(xfade)
    print(xfade.dtype, src.dtype, diff.dtype, dest.dtype)
